...
}]),angular.module("template/carousel/slide.html",[]).run(["$templateCache",function($templateCache){$templateCache.put("template/carousel/slide.html","<div ng-class=\"{\n    'active': leaving || (active && !entering),\n    'prev': (next || active) && direction=='prev',\n    'next': (next || active) && direction=='next',\n    'right': direction=='prev',\n    'left': direction=='next'\n  }\" class=\"item text-center\" ng-transclude></div>\n")}]),angular.module("template/datepicker/datepicker.html",[]).run(["$templateCache",function($templateCache){$templateCache.put("template/datepicker/datepicker.html",'<table>\n  <thead>\n    <tr>\n      <th><button type="button" class="btn btn-default btn-sm pull-left" ng-click="move(-1)"><i class="glyphicon glyphicon-chevron-left"></i></button></th>\n      <th colspan="{{rows[0].length - 2 + showWeekNumbers}}"><button type="button" class="btn btn-default btn-sm btn-block" ng-click="toggleMode()"><strong>{{title}}</strong></button></th>\n      <th><button type="button" class="btn btn-default btn-sm pull-right" ng-click="move(1)"><i class="glyphicon glyphicon-chevron-right"></i></button></th>\n    </tr>\n    <tr ng-show="labels.length > 0" class="h6">\n      <th ng-show="showWeekNumbers" class="text-center">#</th>\n      <th ng-repeat="label in labels" class="text-center">{{label}}</th>\n    </tr>\n  </thead>\n  <tbody>\n    <tr ng-repeat="row in rows">\n      <td ng-show="showWeekNumbers" class="text-center"><em>{{ getWeekNumber(row) }}</em></td>\n      <td ng-repeat="dt in row" class="text-center">\n        <button type="button" style="width:100%;" class="btn btn-default btn-sm" ng-class="{\'btn-info\': dt.selected}" ng-click="select(dt.date)" ng-disabled="dt.disabled"><span ng-class="{\'text-muted\': dt.secondary}">{{dt.label}}</span></button>\n      </td>\n    </tr>\n  </tbody>\n</table>\n')}]),angular.module("template/datepicker/popup.html",[]).run(["$templateCache",function($templateCache){$templateCache.put("template/datepicker/popup.html","<ul class=\"dropdown-menu\" ng-style=\"{display: (isOpen && 'block') || 'none', top: position.top+'px', left: position.left+'px'}\">\n	<li ng-transclude></li>\n"+'	<li ng-show="showButtonBar" style="padding:10px 9px 2px">\n		<span class="btn-group">\n			<button type="button" class="btn btn-sm btn-info" ng-click="today()">{{currentText}}</button>\n			<button type="button" class="btn btn-sm btn-default" ng-click="showWeeks = ! showWeeks" ng-class="{active: showWeeks}">{{toggleWeeksText}}</button>\n			<button type="button" class="btn btn-sm btn-danger" ng-click="clear()">{{clearText}}</button>\n		</span>\n		<button type="button" class="btn btn-sm btn-success pull-right" ng-click="isOpen = false">{{closeText}}</button>\n	</li>\n</ul>\n')}]),angular.module("template/modal/backdrop.html",[]).run(["$templateCache",function($templateCache){$templateCache.put("template/modal/backdrop.html",'<div class="modal-backdrop fade" ng-class="{in: animate}" ng-style="{\'z-index\': 1040 + index*10}"></div>')}]),angular.module("template/modal/window.html",[]).run(["$templateCache",function($templateCache){$templateCache.put("template/modal/window.html",'<div tabindex="-1" class="modal fade {{ windowClass }}" ng-class="{in: animate}" ng-style="{\'z-index\': 1050 + index*10, display: \'block\'}" ng-click="close($event)">\n    <div class="modal-dialog"><div class="modal-content" ng-transclude></div></div>\n</div>')}]),angular.module("template/pagination/pager.html",[]).run(["$templateCache",function($templateCache){$templateCache.put("template/pagination/pager.html",'<ul class="pager">\n  <li ng-repeat="page in pages" ng-class="{disabled: page.disabled, previous: page.previous, next: page.next}"><a ng-click="selectPage(page.number)">{{page.text}}</a></li>\n</ul>')}]),angular.module("template/pagination/pagination.html",[]).run(["$templateCache",function($templateCache){$templateCache.put("template/pagination/pagination.html",'<ul class="pagination">\n  <li ng-repeat="page in pages" ng-class="{active: page.active, disabled: page.disabled}"><a ng-click="selectPage(page.number)">{{page.text}}</a></li>\n</ul>')}]),angular.module("template/tooltip/tooltip-html-unsafe-popup.html",[]).run(["$templateCache",function($templateCache){$templateCache.put("template/tooltip/tooltip-html-unsafe-popup.html",'<div class="tooltip {{placement}}" ng-class="{ in: isOpen(), fade: animation() }">\n  <div class="tooltip-arrow"></div>\n  <div class="tooltip-inner" bind-html-unsafe="content"></div>\n</div>\n')}]),angular.module("template/tooltip/tooltip-popup.html",[]).run(["$templateCache",function($templateCache){$templateCache.put("template/tooltip/tooltip-popup.html",'<div class="tooltip {{placement}}" ng-class="{ in: isOpen(), fade: animation() }">\n  <div class="tooltip-arrow"></div>\n  <div class="tooltip-inner" ng-bind="content"></div>\n</div>\n')}]),angular.module("template/popover/popover.html",[]).run(["$templateCache",function($templateCache){$templateCache.put("template/popover/popover.html",'<div class="popover {{placement}}" ng-class="{ in: isOpen(), fade: animation() }">\n  <div class="arrow"></div>\n\n  <div class="popover-inner">\n      <h3 class="popover-title" ng-bind="title" ng-show="title"></h3>\n      <div class="popover-content" ng-bind="content"></div>\n  </div>\n</div>\n')}]),angular.module("template/progressbar/bar.html",[]).run(["$templateCache",function($templateCache){$templateCache.put("template/progressbar/bar.html",'<div class="progress-bar" ng-class="type && \'progress-bar-\' + type" ng-transclude></div>')}]),angular.module("template/progressbar/progress.html",[]).run(["$templateCache",function($templateCache){$templateCache.put("template/progressbar/progress.html",'<div class="progress" ng-transclude></div>')}]),angular.module("template/progressbar/progressbar.html",[]).run(["$templateCache",function($templateCache){$templateCache.put("template/progressbar/progressbar.html",'<div class="progress"><div class="progress-bar" ng-class="type && \'progress-bar-\' + type" ng-transclude></div></div>')}]),angular.module("template/rating/rating.html",[]).run(["$templateCache",function($templateCache){$templateCache.put("template/rating/rating.html",'<span ng-mouseleave="reset()">\n    <i ng-repeat="r in range" ng-mouseenter="enter($index + 1)" ng-click="rate($index + 1)" class="glyphicon" ng-class="$index < val && (r.stateOn || \'glyphicon-star\') || (r.stateOff || \'glyphicon-star-empty\')"></i>\n</span>')}]),angular.module("template/tabs/tab.html",[]).run(["$templateCache",function($templateCache){$templateCache.put("template/tabs/tab.html",'<li ng-class="{active: active, disabled: disabled}">\n  <a ng-click="select()" tab-heading-transclude>{{heading}}</a>\n</li>\n')}]),angular.module("template/tabs/tabset-titles.html",[]).run(["$templateCache",function($templateCache){$templateCache.put("template/tabs/tabset-titles.html","<ul class=\"nav {{type && 'nav-' + type}}\" ng-class=\"{'nav-stacked': vertical}\">\n</ul>\n")}]),angular.module("template/tabs/tabset.html",[]).run(["$templateCache",function($templateCache){$templateCache.put("template/tabs/tabset.html",'\n<div class="tabbable">\n  <ul class="nav {{type && \'nav-\' + type}}" ng-class="{\'nav-stacked\': vertical, \'nav-justified\': justified}" ng-transclude></ul>\n  <div class="tab-content">\n    <div class="tab-pane" \n         ng-repeat="tab in tabs" \n         ng-class="{active: tab.active}"\n         tab-content-transclude="tab">\n    </div>\n  </div>\n</div>\n')}]),angular.module("template/timepicker/timepicker.html",[]).run(["$templateCache",function($templateCache){$templateCache.put("template/timepicker/timepicker.html",'<table>\n	<tbody>\n		<tr class="text-center">\n			<td><a ng-click="incrementHours()" class="btn btn-link"><span class="glyphicon glyphicon-chevron-up"></span></a></td>\n			<td>&nbsp;</td>\n			<td><a ng-click="incrementMinutes()" class="btn btn-link"><span class="glyphicon glyphicon-chevron-up"></span></a></td>\n			<td ng-show="showMeridian"></td>\n		</tr>\n		<tr>\n			<td style="width:50px;" class="form-group" ng-class="{\'has-error\': invalidHours}">\n				<input type="text" ng-model="hours" ng-change="updateHours()" class="form-control text-center" ng-mousewheel="incrementHours()" ng-readonly="readonlyInput" maxlength="2">\n			</td>\n			<td>:</td>\n			<td style="width:50px;" class="form-group" ng-class="{\'has-error\': invalidMinutes}">\n				<input type="text" ng-model="minutes" ng-change="updateMinutes()" class="form-control text-center" ng-readonly="readonlyInput" maxlength="2">\n			</td>\n			<td ng-show="showMeridian"><button type="button" class="btn btn-default text-center" ng-click="toggleMeridian()">{{meridian}}</button></td>\n		</tr>\n		<tr class="text-center">\n			<td><a ng-click="decrementHours()" class="btn btn-link"><span class="glyphicon glyphicon-chevron-down"></span></a></td>\n			<td>&nbsp;</td>\n			<td><a ng-click="decrementMinutes()" class="btn btn-link"><span class="glyphicon glyphicon-chevron-down"></span></a></td>\n			<td ng-show="showMeridian"></td>\n		</tr>\n	</tbody>\n</table>\n')}]),angular.module("template/typeahead/typeahead-match.html",[]).run(["$templateCache",function($templateCache){$templateCache.put("template/typeahead/typeahead-match.html",'<a tabindex="-1" bind-html-unsafe="match.label | typeaheadHighlight:query"></a>')}]),angular.module("template/typeahead/typeahead-popup.html",[]).run(["$templateCache",function($templateCache){$templateCache.put("template/typeahead/typeahead-popup.html","<ul class=\"dropdown-menu\" ng-style=\"{display: isOpen()&&'block' || 'none', top: position.top+'px', left: position.left+'px'}\">\n"+'    <li ng-repeat="match in matches" ng-class="{active: isActive($index) }" ng-mouseenter="selectActive($index)" ng-click="selectMatch($index)">\n        <div typeahead-match index="$index" match="match" query="query" template-url="templateUrl"></div>\n    </li>\n</ul>')}]),+function($){var Collapse=function(element,options){this.$element=$(element),this.options=$.extend({},Collapse.DEFAULTS,options),this.transitioning=null,this.options.parent&&(this.$parent=$(this.options.parent)),this.options.toggle&&this.toggle()};Collapse.DEFAULTS={toggle:!0},Collapse.prototype.dimension=function(){var hasWidth=this.$element.hasClass("width");return hasWidth?"width":"height"},Collapse.prototype.show=function(){if(!this.transitioning&&!this.$element.hasClass("in")){var startEvent=$.Event("show.bs.collapse");if(this.$element.trigger(startEvent),!startEvent.isDefaultPrevented()){var actives=this.$parent&&this.$parent.find("> .panel > .in");if(actives&&actives.length){var hasData=actives.data("bs.collapse");if(hasData&&hasData.transitioning)return;actives.collapse("hide"),hasData||actives.data("bs.collapse",null)}var dimension=this.dimension();this.$element.removeClass("collapse").addClass("collapsing")[dimension](0),this.transitioning=1;var complete=function(){this.$element.removeClass("collapsing").addClass("in")[dimension]("auto"),this.transitioning=0,this.$element.trigger("shown.bs.collapse")};if(!$.support.transition)return complete.call(this);var scrollSize=$.camelCase(["scroll",dimension].join("-"));this.$element.one($.support.transition.end,$.proxy(complete,this)).emulateTransitionEnd(350)[dimension](this.$element[0][scrollSize])}}},Collapse.prototype.hide=function(){if(!this.transitioning&&this.$element.hasClass("in")){var startEvent=$.Event("hide.bs.collapse");if(this.$element.trigger(startEvent),!startEvent.isDefaultPrevented()){var dimension=this.dimension();this.$element[dimension](this.$element[dimension]())[0].offsetHeight,this.$element.addClass("collapsing").removeClass("collapse").removeClass("in"),this.transitioning=1;var complete=function(){this.transitioning=0,this.$element.trigger("hidden.bs.collapse").removeClass("collapsing").addClass("collapse")};return $.support.transition?void this.$element[dimension](0).one($.support.transition.end,$.proxy(complete,this)).emulateTransitionEnd(350):complete.call(this)}}},Collapse.prototype.toggle=function(){this[this.$element.hasClass("in")?"hide":"show"]()};var old=$.fn.collapse;$.fn.collapse=function(option){return this.each(function(){var $this=$(this),data=$this.data("bs.collapse"),options=$.extend({},Collapse.DEFAULTS,$this.data(),"object"==typeof option&&option);data||$this.data("bs.collapse",data=new Collapse(this,options)),"string"==typeof option&&data[option]()})},$.fn.collapse.Constructor=Collapse,$.fn.collapse.noConflict=function(){return $.fn.collapse=old,this},$(document).on("click.bs.collapse.data-api","[data-toggle=collapse]",function(e){var href,$this=$(this),target=$this.attr("data-target")||e.preventDefault()||(href=$this.attr("href"))&&href.replace(/.*(?=#[^\s]+$)/,""),$target=$(target),data=$target.data("bs.collapse"),option=data?"toggle":$this.data(),parent=$this.attr("data-parent"),$parent=parent&&$(parent);data&&data.transitioning||($parent&&$parent.find('[data-toggle=collapse][data-parent="'+parent+'"]').not($this).addClass("collapsed"),$this[$target.hasClass("in")?"addClass":"removeClass"]("collapsed")),$target.collapse(option)})}(jQuery),+function($){function transitionEnd(){var el=document.createElement("bootstrap"),transEndEventNames={WebkitTransition:"webkitTransitionEnd",MozTransition:"transitionend",OTransition:"oTransitionEnd otransitionend",transition:"transitionend"};for(var name in transEndEventNames)if(void 0!==el.style[name])return{end:transEndEventNames[name]}}$.fn.emulateTransitionEnd=function(duration){var called=!1,$el=this;$(this).one($.support.transition.end,function(){called=!0});var callback=function(){called||$($el).trigger($.support.transition.end)};return setTimeout(callback,duration),this},$(function(){$.support.transition=transitionEnd()})}(jQuery),+function($){function clearMenus(){$(backdrop).remove(),$(toggle).each(function(e){var $parent=getParent($(this));$parent.hasClass("open")&&($parent.trigger(e=$.Event("hide.bs.dropdown")),e.isDefaultPrevented()||$parent.removeClass("open").trigger("hidden.bs.dropdown"))})}function getParent($this){var selector=$this.attr("data-target");selector||(selector=$this.attr("href"),selector=selector&&/#/.test(selector)&&selector.replace(/.*(?=#[^\s]*$)/,""));var $parent=selector&&$(selector);return $parent&&$parent.length?$parent:$this.parent()}var backdrop=".dropdown-backdrop",toggle="[data-toggle=dropdown]",Dropdown=function(element){$(element).on("click.bs.dropdown",this.toggle)};Dropdown.prototype.toggle=function(e){var $this=$(this);if(!$this.is(".disabled, :disabled")){var $parent=getParent($this),isActive=$parent.hasClass("open");if(clearMenus(),!isActive){if("ontouchstart"in document.documentElement&&!$parent.closest(".navbar-nav").length&&$('<div class="dropdown-backdrop"/>').insertAfter($(this)).on("click",clearMenus),$parent.trigger(e=$.Event("show.bs.dropdown")),e.isDefaultPrevented())return;$parent.toggleClass("open").trigger("shown.bs.dropdown"),$this.focus()}return!1}},Dropdown.prototype.keydown=function(e){if(/(38|40|27)/.test(e.keyCode)){var $this=$(this);if(e.preventDefault(),e.stopPropagation(),!$this.is(".disabled, :disabled")){var $parent=getParent($this),isActive=$parent.hasClass("open");if(!isActive||isActive&&27==e.keyCode)return 27==e.which&&$parent.find(toggle).focus(),$this.click();var $items=$("[role=menu] li:not(.divider):visible a",$parent);if($items.length){var index=$items.index($items.filter(":focus"));38==e.keyCode&&index>0&&index--,40==e.keyCode&&index<$items.length-1&&index++,~index||(index=0),$items.eq(index).focus()}}}};var old=$.fn.dropdown;$.fn.dropdown=function(option){return this.each(function(){var $this=$(this),data=$this.data("bs.dropdown");data||$this.data("bs.dropdown",data=new Dropdown(this)),"string"==typeof option&&data[option].call($this)})},$.fn.dropdown.Constructor=Dropdown,$.fn.dropdown.noConflict=function(){return $.fn.dropdown=old,this},$(document).on("click.bs.dropdown.data-api",clearMenus).on("click.bs.dropdown.data-api",".dropdown form",function(e){e.stopPropagation()}).on("click.bs.dropdown.data-api",toggle,Dropdown.prototype.toggle).on("keydown.bs.dropdown.data-api",toggle+", [role=menu]",Dropdown.prototype.keydown)}(jQuery),+function($){var dismiss='[data-dismiss="alert"]',Alert=function(el){$(el).on("click",dismiss,this.close)};Alert.prototype.close=function(e){function removeElement(){$parent.trigger("closed.bs.alert").remove()}var $this=$(this),selector=$this.attr("data-target");selector||(selector=$this.attr("href"),selector=selector&&selector.replace(/.*(?=#[^\s]*$)/,""));var $parent=$(selector);e&&e.preventDefault(),$parent.length||($parent=$this.hasClass("alert")?$this:$this.parent()),$parent.trigger(e=$.Event("close.bs.alert")),e.isDefaultPrevented()||($parent.removeClass("in"),$.support.transition&&$parent.hasClass("fade")?$parent.one($.support.transition.end,removeElement).emulateTransitionEnd(150):removeElement())};var old=$.fn.alert;$.fn.alert=function(option){return this.each(function(){var $this=$(this),data=$this.data("bs.alert");data||$this.data("bs.alert",data=new Alert(this)),"string"==typeof option&&data[option].call($this)})},$.fn.alert.Constructor=Alert,$.fn.alert.noConflict=function(){return $.fn.alert=old,this},$(document).on("click.bs.alert.data-api",dismiss,Alert.prototype.close)}(jQuery),angular.module("gymwithmusic",["youtube","ngCookies","ngResource","ui.bootstrap","ui.router","gymwithmusic.system","ngAnimate"]),angular.module("gymwithmusic.system",[]),angular.module("gymwithmusic").config(["$stateProvider","$urlRouterProvider",function($stateProvider,$urlRouterProvider){$stateProvider.state("home",{url:"/",templateUrl:"/views/index.html"}).state("admin host",{url:"/admin/screen",templateUrl:"/views/admin/screen.html",data:{rule:function(user){return user.admin?!0:!1}}}),$urlRouterProvider.otherwise("/")}]),angular.module("gymwithmusic").config(["$locationProvider",function($locationProvider){$locationProvider.html5Mode(!0)}]),angular.module("gymwithmusic.system").controller("HeaderController",["$scope","Global","$location",function($scope,Global,$location){$scope.global=Global,"/signin"!==$location.path()&&"/signup"!==$location.path()&&angular.element("html").removeClass("auth"),"undefined"!=typeof Global.user&&null!==Global.user&&Global.user.admin&&($scope.menu=[{title:"Word host",link:"admin/screen"}]),$scope.$on("$locationChangeSuccess",function(){"/admin/screen"===$location.path()?angular.element("html").addClass("admin"):angular.element("html").removeClass("admin")}),$scope.isCollapsed=!1}]),angular.module("gymwithmusic.system").controller("IndexController",["$scope","Global","$location",function($scope,Global,$location){$scope.global=Global,$scope.$watch(function(){return $location.path()},function(newValue){Global.user||"/admin/screen"!==newValue||(window.location="/"),0==Global.user.admin&&"/admin/screen"===newValue&&(window.location="/")})}]),angular.module("gymwithmusic.system").controller("MessageController",["$scope","Global",function($scope,Global){$scope.global=Global,$scope.global.messages=[],$scope.closeMessage=function(index){$scope.global.messages.splice(index,1)}}]),angular.module("gymwithmusic.system").controller("ScreenController",["$scope","$http","Global","Faye","$youtube","$location",function($scope,$http,Global,Faye,$youtube){function newVideo(){$http.get("/videos").success(function(data){if(data.videos.length>0){var currentVideo=data.videos[0];Global.currentVideo=currentVideo;var video={_id:currentVideo._id,added_by:currentVideo.added_by._id,thumbnail:currentVideo.thumbnail,title:currentVideo.title,url:currentVideo.url,youtube_id:currentVideo.youtube_id};$http.post("/videos/current",video).success(function(data){"success"===data.status?($youtube.playerId="best-vid",$youtube.videoId=currentVideo.youtube_id,$youtube.loadPlayer(),$scope.skipVotes=[],Faye.publish("/currentVideo",currentVideo),Faye.publish("/videos",data.message),Faye.publish("/skipVotes",$scope.skipVotes)):console.log(data.message)}).error(function(){console.log("Unknown error")})}else Global.currentVideo={title:"Geen videos op dit moment, voeg er een toe!",added_by:{name:""}},Faye.publish("/currentVideo",Global.currentVideo),Faye.subscribe("/videos",function(){Global.currentVideo.youtube_id||$scope.$apply(function(){newVideo()})})})}$scope.global=Global,function(){$http.get("/skipvotes").success(function(data){$scope.skipVotes=data.votes})}(),Faye.subscribe("/skipVotes",function(votes){var votes=votes;$scope.$apply(function(){$scope.skipVotes=votes}),void 0!==votes[0]&&votes[0].voters.length>=5&&(newVideo(),$scope.skipVotes=[],Faye.publish("/skipVotes",$scope.skipVotes))}),newVideo(),$scope.$on("youtube.player.ready",function(){$youtube.player.seekTo(.5,!0),$youtube.player.playVideo()}),$scope.$on("youtube.player.ended",function(){newVideo()})}]),angular.module("gymwithmusic.system").controller("VideoController",["$scope","Global","$location","$http","Faye",function($scope,Global,$location,$http,Faye){function convert_time(duration){var a=duration.match(/\d+/g);return duration.indexOf("M")>=0&&-1==duration.indexOf("H")&&-1==duration.indexOf("S")&&(a=[0,a[0],0]),duration.indexOf("H")>=0&&-1==duration.indexOf("M")&&(a=[a[0],0,a[1]]),duration.indexOf("H")>=0&&-1==duration.indexOf("M")&&-1==duration.indexOf("S")&&(a=[a[0],0,0]),duration=0,3==a.length&&(duration+=3600*parseInt(a[0]),duration+=60*parseInt(a[1]),duration+=parseInt(a[2])),2==a.length&&(duration+=60*parseInt(a[0]),duration+=parseInt(a[1])),1==a.length&&(duration+=parseInt(a[0])),duration}$scope.global=Global,function(){$http.get("/videos").success(function(data){Global.allVideos=data.videos,function(){$http.get("/videos/current").success(function(data){(Global.allVideos.length>0||""!==Global.currentVideo.added_by.name)&&(Global.currentVideo=data.video[0])})}()})}(),function(){$http.get("/skipvotes").success(function(data){$scope.skipVotes=data.votes})}(),Faye.subscribe("/videos",function(videos){$scope.$apply(function(){Global.allVideos=videos})}),Faye.subscribe("/currentVideo",function(video){$scope.$apply(function(){Global.currentVideo=video})}),Faye.subscribe("/skipVotes",function(votes){var votes=votes;$scope.$apply(function(){$scope.skipVotes=votes})}),$scope.voteSkip=function(){var vote={userId:Global.user._id};$http.post("/skipvotes",vote).success(function(data){Faye.publish("/skipVotes",data.votes)}).error(function(){$scope.global.messages.push({type:"danger",msg:"Oeps! Er liep iets fout bij het registreren van je stem."})})},$scope.addByUrl=function(url){var regExp=/^.*(youtu.be\/|v\/|u\/\w\/|embed\/|watch\?v=|\&v=)([^#\&\?]*).*/,match=url.match(regExp);if(match&&11===match[2].length){var id=match[2];this.addById(id)}else $scope.global.messages.push({type:"danger",msg:"Oeps! Foutje in de url. YouTube staat enkel de standaard url toe: http://www.youtube.com/watch?v=VIDEO_ID."})},$scope.autocomplete=function(terms){return $http.jsonp("http://suggestqueries.google.com/complete/search?hl=en&ds=yt&client=youtube&hjson=t&cp=1&q="+terms+"&key=AIzaSyDbhofgKeHR6BvGATtxqGv1WIJMSaCl5sM&format=5&alt=json&callback=JSON_CALLBACK").then(function(response){var results=[];return angular.forEach(response.data[1],function(item){results.push(item[0])}),results})},$scope.searchYoutube=function(terms){""!==terms&&void 0!==terms?$http.get("https://www.googleapis.com/youtube/v3/search?part=snippet&maxResults=5&order=relevance&q="+terms+"&type=video&videoCategoryId=10&videoEmbeddable=true&key=AIzaSyDbhofgKeHR6BvGATtxqGv1WIJMSaCl5sM").success(function(data){$scope.searchResults=data.items}).error(function(){$scope.global.messages.push({type:"danger",msg:"Oeps! Er is iets misgelopen bij YouTube, probeer nog eens?"})}):$scope.searchResults=[]},$scope.addById=function(id,$event){$http.get("https://www.googleapis.com/youtube/v3/videos?part=snippet,contentDetails&id="+id+"&key=AIzaSyDbhofgKeHR6BvGATtxqGv1WIJMSaCl5sM").success(function(data){var duration=convert_time(data.items[0].contentDetails.duration);if(900>=duration){var title=data.items[0].snippet.title,thumbnail=data.items[0].snippet.thumbnails.high.url,video={title:title,url:"http://www.youtube.com/watch?v="+id,youtube_id:id,thumbnail:thumbnail,added_by:Global.user._id};$http.post("/videos",video).success(function(data){"success"===data.status?($event&&angular.element($event.toElement).addClass("added"),$scope.global.messages.push({type:"success",msg:"Je video is toegevoegd"}),Faye.publish("/videos",data.message)):($scope.global.messages.push({type:"danger",msg:data.message}),$event&&angular.element($event.toElement).addClass("added"))}).error(function(){$scope.global.messages.push({type:"danger",msg:"Oeps! De video werd gevonden maar er liep iets fout bij ons. Probeer opnieuw?"})})}else $scope.global.messages.push({type:"danger",msg:"Je video is te lang! We hebben graag verschillende nummers!"})}).error(function(){$scope.global.messages.push({type:"danger",msg:"Oeps! YouTube kon geen video vinden, probeer opnieuw?"})})},$scope.vote=function(id){if(Global.user){var data={userId:Global.user._id,userName:Global.user.name,userEmail:Global.user.email};$http.post("/videos/"+id,data).success(function(data){Faye.publish("/videos",data.videos)}).error(function(){$scope.global.messages.push({type:"danger",msg:"Oeps! Er liep iets fout tijdens het registreren van je stem."})})}else $scope.global.messages.push({type:"danger",msg:"Je moet inloggen om te stemmen!"})},$scope.delete=function(id){var data={videoId:id};$http.delete("/videos/"+id,data).success(function(data){Faye.publish("/videos",data.videos)}).error(function(){$scope.global.messages.push({type:"danger",msg:"Er liep iets fout bij het verwijderen van de video."})})},$scope.checkIfAdded=function(obj,videoId){var returnKey=-1;return angular.forEach(obj,function(video,key){return video.youtube_id===videoId?(returnKey=key,!1):void 0}),returnKey}}]),angular.module("youtube",["ng"]).run(function(){}).service("$youtube",["$window","$rootScope",function($window,$rootScope){function applyBroadcast(event){$rootScope.$apply(function(){$rootScope.$broadcast(event)})}function onPlayerReady(){applyBroadcast(eventPrefix+"ready")}function onPlayerStateChange(event){var state=stateNames[event.data];void 0!==typeof state&&applyBroadcast(eventPrefix+state)}var service={ready:!0,playerId:null,player:null,videoId:null,playerHeight:"405",playerWidth:"675",createPlayer:function(){return new YT.Player(this.playerId,{height:this.playerHeight,width:this.playerWidth,videoId:this.videoId,playerVars:{autohide:1,iv_load_policy:3},events:{onReady:onPlayerReady,onStateChange:onPlayerStateChange}})},loadPlayer:function(){this.ready&&this.playerId&&this.videoId&&(this.player&&"function"==typeof this.player.destroy&&this.player.destroy(),this.player=this.createPlayer())}},stateNames={0:"ended",1:"playing",2:"paused",3:"buffering",5:"queued"},eventPrefix="youtube.player.";return $window.onYouTubeIframeAPIReady=function(){$rootScope.$apply(function(){service.ready=!0})},service}]).directive("youtubePlayer",["$youtube",function($youtube){return{restrict:"EA",scope:{videoId:"="},link:function(scope,element){$youtube.playerId=element[0].id,scope.player=$youtube;var stopWatchingReady=scope.$watch("player.ready",function(ready){ready&&(stopWatchingReady(),scope.$watch("videoId",function(id){$youtube.videoId=id,$youtube.loadPlayer()}))})}}}]),angular.module("ng").filter("cut",function(){return function(value,wordwise,max,tail){if(!value)return"";if(max=parseInt(max,10),!max)return value;if(value.length<=max)return value;if(value=value.substr(0,max),wordwise){var lastspace=value.lastIndexOf(" ");-1!==lastspace&&(value=value.substr(0,lastspace))}return value+(tail||" …")}}),angular.element(document).ready(function(){"#_=_"===window.location.hash&&(window.location.hash=""),angular.bootstrap(document,["gymwithmusic"])}),angular.module("gymwithmusic.system").factory("Global",[function(){var _this=this;return _this._data={user:window.user,authenticated:!!window.user},_this._data}]);var FayeServerURL="/socket";angular.module("gymwithmusic.system").factory("Faye",function(){var client=new window.Faye.Client(FayeServerURL);return{publish:function(channel,message){client.publish(channel,message)},subscribe:function(channel,callback){client.subscribe(channel,callback)}}});